...
import { GitMerge, Database, Link2, Info, ArrowRight } from 'lucide-react';
import { parseSqlJoins, JoinRelation } from '../utils/sqlParser.ts';
...
export const JoinVisualizer: React.FC<JoinVisualizerProps> = ({ sql }) => {
  ...
  // If there's no FROM or no tables parsed, don't show anything
    ...
    <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-4 shadow-lg mb-4 animate-in fade-in duration-300">
      <div className="flex items-center justify-between mb-3 border-b border-slate-700 pb-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-100">テーブル結合 (JOIN) 構造マップ</h3>
        </div>
        {isMultiDb && (
      ...
      {joins.length === 0 ? (
        <div className="flex items-center gap-2 text-xs text-slate-400 py-1 bg-slate-900/40 px-3 rounded-lg">
          <Info size={14} className="text-slate-500" />
          <span>クエリ内にテーブル結合（JOIN）は定義されていません。単一テーブル {`"${tables[0]}"`} の参照です。</span>
        </div>
      ) : (
        <div className="flex flex-col gap-4 py-2">
          ...
          <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl font-mono text-xs text-slate-300">
            <div className="text-[10px] text-slate-500 uppercase font-semibold mb-2 select-none tracking-wider">結合ツリー図 (Tree View)</div>
                ...
                if (i === 0) {
                  return (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-slate-500 italic">{nodeType} (起点となるメインテーブル)</span>
                    </div>
                  );
                }
